...
import { View, Text, Image, FlatList, TouchableOpacity, Dimensions, StyleSheet, ActivityIndicator, Linking } from "react-native";
...
const PopularDeal = () => {
  ...
  const handleScroll = (event) => {
    const slideIndex = Math.round(event.nativeEvent.contentOffset.x / (CARD_WIDTH + CARD_MARGIN));
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      ...
      <Image source={{ uri: `${process.env.EXPO_PUBLIC_API_BASE_URL}${item.imageLink}` }} style={styles.image} resizeMode="contain" />
    </TouchableOpacity>
  );
